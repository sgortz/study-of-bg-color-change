...
let colors = {
  day: "linear-gradient(120deg, #a1c4fd 0%, #c2e9fb 100%)",
  night: "linear-gradient(to top, #09203f 0%, #537895 100%)",
  rainyOrCloudy: "linear-gradient(to top, #cfd9df 0%, #e2ebf0 100%)",
  sandOrDust: "linear-gradient(135deg, #fdfcfb 0%, #e2d1c3 100%);",
  thunderstorm:
  ...
  mornings: "linear-gradient(to top, #a8edea 0%, #fed6e3 100%)",
};
...
function displayCity(response) {
  ...
  if (apiIconResponse.includes("n")) {
    colorBackground.style.backgroundImage = colors.night;
    colorBackground.style.color = "whitesmoke";
  } else if (response.data.weather[0].main === forecast[0] && hour < 12) {
    colorBackground.style.backgroundImage = colors.mornings;
  } else if (response.data.weather[0].main === forecast[0] && hour >= 12) {
    colorBackground.style.backgroundImage = colors.day;
  } else if (response.data.weather[0].main === forecast[(10, 11)]) {
    colorBackground.style.backgroundImage = colors.sandOrDust;
  } else if (response.data.weather[0].main === forecast[(12, 13)]) {
    colorBackground.style.backgroundImage = colors.thunderstorm;
  } else {
    colorBackground.style.backgroundImage = colors.rainyOrCloudy;
  }
}
